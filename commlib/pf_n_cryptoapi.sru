HA$PBExportHeader$pf_n_cryptoapi.sru
$PBExportComments$$$HEX17$$54c538d654d6200068d518c2200030aea5b22000a8ba4cc7200024c60cbe1dc8b8d2$$ENDHEX$$
forward
global type pf_n_cryptoapi from nonvisualobject
end type
end forward

global type pf_n_cryptoapi from nonvisualobject autoinstantiate
end type
...
Encoding iEncoding		= EncodingAnsi!	// current string encoding option
...
public function integer of_enumproviders (ref unsignedlong aul_providerid[], ref string as_provider[], ref unsignedlong aul_provtype[])
...
//lblob_data = Blob(as_data, iEncoding)
lblob_data = Blob(as_data)
...
//Return String(lblob_decrypted, iEncoding)
Return String(lblob_decrypted)
...
// ----------  --------		-----------------------------------------------------
// 01/18/2011	RolandS		Initial Coding
// 06/10/2013	RolandS		Added call to CryptGetHashParam to determine size
//									of the buffer needed to hold the hash value.
// -----------------------------------------------------------------------------
...
// Get the hash value
lblob_hash = Blob(Space(lul_DataLen), iEncoding)
...
public function long of_hex2long (string as_hex);string ls_hex
integer i,length
long result = 0
...
on pf_n_cryptoapi.create
call super::create
TriggerEvent( this, "constructor" )
end on

on pf_n_cryptoapi.destroy
